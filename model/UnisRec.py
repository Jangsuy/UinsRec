...
class UnisRec(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        neg_pairs = self.moe_adaptor(batch_dict['neg_item_ix'])  # b d
        logits = torch.cat([hist_emb.mul(pos_pairs).sum(dim=-1), 
                            hist_emb.mul(neg_pairs).sum(dim=-1)],
                            dim=0)

        
        y = torch.cat([torch.ones(len(pos_pairs), dtype=torch.float32),
                       torch.zeros(len(neg_pairs), dtype=torch.float32)],
                       dim=0)
